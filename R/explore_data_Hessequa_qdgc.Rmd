...
These records were removed for the analysis
...
Number of observations vary across QDGC, a few with ~ 500 observations, but most have > 750 observations, and one has > 1500 observations
...
Total of 38709 observations. Less than from the SABAP2 data (over 100 000 bird records)
...
Number of species varies across QDGC. One QDGQC with > 250 species, and a few with < 100, most > 125 species
...
- Acrocephalus scirpaceus (common reed warbler) (32 records). This species is a rare summer visitor with few records in southern Africa. Possible missidentifications. There are two warblers in the SABAP2 data: Acrocephalus baeticatus (African Reed warbler) and Acrocephalus gracilirostris (marsh warbler)
...
- Phoenicopterus roseus (greater flamingo) (44 records): As with lesser flamingo is a local visitor. Lesser flamingo (Phoeniconaias minor) is in SABAP2
...
- Macronyx capensis (Cape Longclaw) (63 records): Area does appear to be part of range
...
- Psalidoprocne pristoptera (black saw-wing) (176 records): area is within its range
...
- Buteo buteo (buzzard) (221 records): A summer visitor, but it is very similar to Buteo trizonatus (difficult to tell a part), which is a resident in the area, and is in the SABAP2 data. Possible missidentifications?
...
- Certhilauda curvirostris (Cape Lark) (248 records): Range rescricted species, which does not occur in the area. Possible missidentifications? Maybe confused with 	
Certhilauda brevirostris (Agulhas lark), which does occur in this area, and is included in the SABAP2 data.

- Euplectes capensis (yellow bishop) (270 records): area is in range

- Zosterops virens (Cape white-eye) (613 records): two accepted name in GBIF (Zosterops virens Sundevall, 1850 and Zosterops capensis Sundevall, 1850). Area is in range, but it appears under neither name in SABAP2 data.
...
Overall we see an overlap of 57.35%
...
If we look at the graph it appears that of the sabap2 squares the percentage of squares in which a species is observed in sabap2 is strongly positively correlated to the percentage squares in which there is overlap between the sabap2 records and cube records.
...
If we look at the graph it appears that of the sabap2 squares the percentage of squares in which a species is observed in sabap2 is positively correlated to the percentage squares in which a species is recorded in the unstructured data. 
...
For common and very common species (rare excluded) - If we look at the graph it appears that of the SABAP2 squares the percentage of squares in which a species is observed in SABAP2 is positively correlated to the percentage squares in which a species is recorded in the unstructured data. The correlation in weaker than when rare species are included.
...
A few more species (6) are missing from the unstructured data for this time period (i.e., excluding 2023 and 2024)
...
There are 54 species in the unstructured data that are not in the SABAP2 data. For 41 of these species there are < 20 records. For the 13 species for which there are > 20 records, the details are below. These 13 species are discussed above (they were all missing in the 2015-2024 data too). All comparisons below are based on the species in SABAP2.
...
If we look at the graph it appears that of the sabap2 squares the percentage of squares in which a species is observed in sabap2 is strongly positively correlated to the percentage squares in which a species is recorded in the unstructured data. 
...
For common and very common species (rare excluded) - If we look at the graph it appears that of the sabap2 squares the percentage of squares in which a species is observed in sabap2 is positively correlated to the percentage squares in which a species is recorded in the unstructured data. But the correlation is stronger when all species are included.
...
Graph shows for each species the degree to which the number of records over time in SABAP2 correlates with number of records in unstructured dataset. The strength and direction of the correlation varies significantly between species, but for most is strongly negative.
...
Graph shows for each species the degree to which the number of records over time (based on time periods) in SABAP2 correlates with number of records in unstructured dataset. The strength and direction of the correlation varies between species, and for 38 species the correlation is NA. But, for > 20 species correlation = 1.
...
No correlation for any groups. Note for many very rare species correlation was not possible. Species with positive, strong correlations included very rare, rare, common, and very common species.
...
Kappa value = 0.0553 so there is a 'poor' agreement in trends in number of records over time
...
bray_curtis # 0.49... very different
```
