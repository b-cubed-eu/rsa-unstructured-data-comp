---
title: "Explore and compare structured and unstructured data for the Western Cape at pentad resolution"
...
```{r}
# Read SABAP2 Western Cape sampling framework (5min x 5 min/ 8.2 km x 8.2 km)
WCSF <- read_sf(file.path(data_path, "sampling_framework_SABAP2_pentad_WC.gpkg"))
...
```{r}
# Read in unstructured Western Cape bird data (5min x 5 min/ 8.2 km x 8.2 km)
...
Data collected between 2008 and 2024. Mean  of ~2 surveys per year per pentad
...
Number of surveys varies across the pentads, a few with > 500 surveys, most at < 30 surveys.
...
SABAP2WC %>%
  ...
  geom_bar(fill = "#1B9E77") + labs(x = "Year",
...
Number of observations vary across pentads, a few with > 2000 observations, but most have < 550 observations
...
Number of species recorded varies per pentad. Most pentads have between 30 and 150 species recorded, few have > 200 species recorded.
...
Unstructured data from GBIF for birds recorded in pentads of Western Cape between 2008 and 2024. 
...
Total of 1 645 858 observations. Less than from the SABAP2 data
...
The number of records was around or less than 50 000 per year  between 2008 and 2014, but increased to between ~80 000 and 130 000 per year for 2015-2021, increased to ~ 300 000 in 2022, and ~ 400 000 in 2024, and decreased to < 50 000 in 2024. The downloaded data comes from 9 publishers, most are from Cornell Lab of Ornithology (i.e., Ebird), with the second most from iNaturalist. Ebird data dominates all years from 2010, but appears to have not been submitted to GBIF for 2024, which explains the major decrease in records in 2024. Number of records from both iNaturalist and ebird increased from 2022.
...
636 bird species recorded, of which 154 were recorded < 10 times (very rare), 128 were recorded between 10 and 100 times (rare), 130 were recorded between 100 and 1000 times (common), 171 were recorded between 1000 and 10 000 times, and 53 were recorded more than 10 000 times. 134 more bird species recorded than in SABAP2.
...
Number of observations vary across pentads. A few pentads have > 32 000 observations, but most < 100 observations
...
Number of species varies across pentads. Most have < 50 species, but ~40 have > 200 species.

# Comparing structured and unstructured data (2008-2024)

Note the SABAP2 dataset used below contains only pentads for the Western Cape (1842 pentads, according to the sampling framework), data from 2008 to 2023, and the geometry and NA species have been removed. Data for 2023 are incomplete. There are also only records in 1681 pentads.

The unstructured data used below contains only pentads for the Western Cape (1842 pentads, according to the SABAP2 sampling framework), data from 2008 to 2024, and the geometry has been removed. Note data for 2024 are incomplete. There are also only records in 1469 pentads.
...
493 species in both datasets. 98% of species in SABAP2 dataset are in the unstructured data
...
Of the 502 species in the SABAP2 data, 493 are in the unstructured data (98%). Of the 493, 79 are very rare, 80 are rare, 94 are common, 161 very common, and 79 are extremely common. Of the 9 species in SABAP2 that are missing from the unstructured data 7 are very rare, 1 rare, none are common, 1 is very common, and none is extremely common. Therefore, species seemed more likely to not be recorded if very rare/rare.
...
78% of the species in the unstructured data are in SABAP2
...
There are 143 species in the unstructured data that are not in the SABAP2 data. For 126 of these species there are <= 65 records. For the 17 species for which there are > 65 records, the details are below. However, of these 17, a number seem to be missidentifications or there are multiple accepted species names in GBIF. For a number, however, there seems to be an issue with the data flowing into GBIF, as these species should be included in the SABAP2 data. Note the SABAP2 data does go through quality control. All comparisons below are based on the species in SABAP2.

- *Chloephaga hybrida* (kelp goose) (178 records): likely missidentifications, as the species does not occur in South Africa.

- *Alectoris chukar* (chukar) (218 records): alien species that is found on Robben Island (inshore island off Western Cape coast). Records elsewhere could be errors. Robben island does fall under Western Cape, so SABAP2 data should include *Alectoris chukar*. This pentad is not included when SABAP2 data are downloaded for Western Cape from GBIF.

- *Anthus nicholsoni* (Nicholson's Pipit) (596 records): Also an accepted species name in GBIF is *Anthus similis* (Jerdon, 1840), for which the common names are Long-billed pipit (in English) and Nicholsonkoester (in Afrikaans).  *Anthus similis* is in the SABAP2 data.

- *Tychaedon coryphoeus* (Karoo scrub-robin) (1106 records): Two accepted species names in GBIF (*Tychaedon coryphoeus* (Lesson, 1831) and *Erythropygia coryphoeus* (Vieillot, 1817)). Appears in SABAP2 under *Erythropygia coryphoeus*.

- *Prinia hypoxantha* (Drakensberg Prinia) (1109 records). Likely missidentifications. Species does not occur in Western Cape.

- *Sylvia layardi* (Layard's Warbler) (1252 records). On SABAP2 website appears under the synonym '*Curruca layardi* (Hartlaub, 1862)', but the species is not under this name in SABAP2 data downloaded from GBIF.

- *Acrocephalus scirpaceus* (common reed warbler) (1525 records). This species is a rare summer visitor with few records in southern Africa, not really from Western Cape. Possible missidentifications?

- *Curruca subcoerulea* (Chestnut-vented Warbler) (2119 records): Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.

- *Macronyx capensis* (Cape Longclaw) (2966 records): Area is part of range. Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.

- *Upupa epops* (Hoopoe) (3050 records): two accepted species names in GBIF (*Upupa epops* Linnaeus, 1758 and *Upupa africana* Bechstein, 1811). The species appears under the name *Upupa africana* in SABAP2 data.

- *Dessonornis caffer* (Cape Robin-Chat) (3159 records): Cape Robin-Chat appears to have two accepted species names in GBIF (*Cossypha caffra* (Linnaeus, 1771) and *Dessonornis caffer* (Linnaeus, 1771)). It is under *Cossypha caffra* in SABAP2.

- *Psalidoprocne pristoptera* (black saw-wing) (3685 records): area is within its range. Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.

- *Buteo buteo* (buzzard) (4143 records). Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.

- *Euplectes capensis* (yellow bishop) (9486 records). Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.

- *Phoenicopterus roseus* (greater flamingo) (9582 records): Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.

- *Phalacrocorax carbo* (common cormorant) (15506 records): possible misidentification. Maybe being confused with *Phalacrocorax capensis* (cape cormorant), which does occur in the area and is in the SABAP2 data.

- *Zosterops virens* (Cape white-eye) (26880 records): two accepted names in GBIF (*Zosterops virens* Sundevall, 1850 and *Zosterops capensis* Sundevall, 1850). Many records in Western Cape for this species on SABAP2 website, but does not appear in data downloaded from GBIF.
...
Overall we see an overlap of 43%
...
Many species have very low percentage range overlap, most have < 50% overlap (39 have < 10% overlap). The outliers with high percentage overlap are a rare/very rare species.
...
### Only common, very common, and extremely common species in SABAP2
...
For all time periods most species have relatively very low percentage range overlap - at ~50% or below. There is an increase over time in this percentage, as more unstructured data becomes available.
...
490 species appear in both the structured and unstructured datasets, this represents 98% of the species recorded in SABAP2.
...
Of the 502 species in the SABAP2 data (all appear in the period from 2015 to 2022), 490 are in the unstructured data (98%). Of the 490, 76 are very rare, 80 are rare, 94 are common, 161 very common, and 79 extremely common. Of the 12 species in SABAP2 that are missing from the unstructured data 10 are very rare, 1 is rare, none are common, 1 is very common, and none are extremely common. Missing species, therefore, tended to be those that are rare/very rare.
...
There are 128 species in the unstructured data that are not in the SABAP2 data. For 111 there are < 65 records. The 17 species with > 65 records are the same species as discussed above for the period 2008-2024. Reasons are discussed above. All comparisons below are based on the species in SABAP2.
...
Overall we see a range overlap of 40% (slightly lower than for 2008-2024)
...
Most species have very low percentage overlap (< 50%), with 47 having < 10% overlap. The outliers with high percentage overlap are rare/very rare species.
...
If we look at the graph it appears that of the sabap2 grid cells the percentage of grid cells in which a species is observed in sabap2 is positively and strongly correlated to the percentage grid cells in which there is overlap between the sabap2 records and cube records.
...
If we look at the graph it appears that of the sabap2 grid cells the percentage of grid cells in which a species is observed in sabap2 is strongly positively correlated to the percentage grid cells in which a species is recorded in the unstructured data. But note these grid cells might not overlap
...
For common, very common species, and extremely common species (very rare/rare excluded) - If we look at the graph it appears that of the sabap2 grid cells the percentage of grid cells in which a species is observed in sabap2 is positively correlated to the percentage grid cells in which a species is recorded in the unstructured data. But these grid cells might not overlap.
...
The correlations are poor, for most species, particularly very and extremely common species are negative.

### Common and very common species in SABAP2

```{r}
# No records per year per species
time_series_Y1Com <- SABAP2WC %>%
  st_drop_geometry() %>%
  filter(category %in% c("Common", "Very common", "Extremely common")) %>%
  group_by(species, year) %>%
  summarize(occurrence = sum(occurrenceStatus == "PRESENT"))
...
```{r}
# No records per year
total_year_2<-UnstrucWC %>%
  st_drop_geometry() %>%
  group_by(year) %>%
  summarize(Totalocc = sum(n))

# No records per year per species
time_series_Y2adj <- UnstrucWC %>%
  st_drop_geometry()  %>%
  group_by(species, year)  %>%
...
Graph shows for each species the degree to which the number of records over time in SABAP2 correlates with the number of records in the unstructured dataset. The strength and direction of the correlation varies between species, but is improved in comparison to when the data are not corrected for total number of records.
...
Graph shows for each species the degree to which the number of records over time in SABAP2 correlates with the number of records in the unstructured dataset. The strength and direction of the correlation varies between species, but is generally improved in comparision to when trends are compared for years.

### Common and very common species in SABAP2

```{r}
# No records per time-period per species
time_series_Cyc1Com <- SABAP2WC %>%
  st_drop_geometry() %>%
  filter(category %in% c("Common", "Very common", "Extremely common")) %>%
  group_by(species, cyclus) %>%
  summarize(occurrence = sum(occurrenceStatus == "PRESENT")) %>%
...
Correlations have improved in comparison to when trends are evaluated across years
...
total_cyc_1<-SABAP2WC %>%
  st_drop_geometry() %>%
  group_by(cyclus) %>%
  summarize(Totalocc = sum(occurrenceStatus == "PRESENT"))

time_series_Cyc1adj <- SABAP2WC %>%
  st_drop_geometry() %>%
  group_by(species, cyclus) %>%
  summarize(occurrence = sum(occurrenceStatus == "PRESENT")) %>%
  filter(cyclus < 5) %>% 
...
total_cyc_2<-UnstrucWC %>%
  st_drop_geometry() %>%
  group_by(cyclus) %>%
  summarize(Totalocc = sum(n))

# No records per time-period per species
time_series_Cyc2adj <- UnstrucWC %>%
  st_drop_geometry()  %>%
  group_by(species, cyclus)  %>%
  summarize(occurrence = sum(n)) %>% 
  filter(cyclus < 5) %>%
...
Correcting for number of observations decreases correlations when data are binned into time periods.
...
K value  = 0.2425, so there is a 'fair' agreement in trends in number of records over time
...
Kappa value = 0.4125, and so there is a 'moderate' agreement in trends in number of records over time
...
Low average occupancy across species (43%). 43% of cells occupied in SABAP2 data are on average occupied in unstructured data
...
Positive and significant correlation between species richness calculated from the two datasets. Is a lot of scatter though
...
Bray curtis index of 0.22 indicates similar species composition
