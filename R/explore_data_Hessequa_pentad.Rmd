...
These recorded were removed for the analysis
...
```{r}
# Remove NAs and geometry, and group years. Used two year internals, as increase in iNat from 2021
...
The number of records was around or less than 2000 per year  between 2015 and 2020, but subsequently increased to between 2500 and 5000 per year for 2021-2023, and decreased to <1000 in 2024. The downloaded data come from 9 publishers, most are from Cornell Lab of Ornithology (i.e., Ebird), with the second most from iNaturalist. Ebird data dominates all years, appears to have not been submitted to GBIF for 2024, which explains the major decrease in records in 2024. Number of records from both iNaturalist and ebird increased from 2021.
...
85% of the species in the unstructured data are in SABAP2
...
- Certhilauda curvirostris (Cape Lark) (58 records): Range rescricted species, which does not occur in the area. Possible missidentifications? Maybe confused with 	
Certhilauda brevirostris (Agulhas lark), which does occur in this area, and is included in the SABAP2 data.
...
- Buteo buteo (buzzard) (111 records): A summer visitor, but it is very similar to Buteo trizonatus (difficult to tell a part), which is a resident in the area, and is in the SABAP2 data. Possible missidentifications?

- Psalidoprocne pristoptera (black saw-wing) (122 records): area is within its range
...
- Euplectes capensis (yellow bishop) (130 records): area is in range

- Zosterops virens (Cape white-eye) (430 records): two accepted name in GBIF (Zosterops virens Sundevall, 1850 and Zosterops capensis Sundevall, 1850). Area is in range, but it appears under neither name in SABAP2 data.
...
The positive correlation between percentage squares in SABAP2 data and cube holds if we look at different time periods. Time period 1 = 2015 and 2016, time period 2 = 2017 and 2018, time period 3 = 2019 and 2020, time period 4 = 2021 and 2022, and time period 5 = 2023 and 2024. Note the results from 2023/2024 look odd as very little SABAP2 and unstructured data submitted to GBIF for this period.
...
265 species appear in both the structured and unstructured datasets, this represents 85%

Of the 310 species in the SABAP2 data (all appear in the period from 2015 to 2022), 265 are in the unstructured data (85%). Of the 165, 37 are very rare, 72 are rare, 126 are common, and 33 very common. Of the 45 species in SABAP2 that are missing from the unstructured data 37 are very rare, 8 are rare, none are common or very common. The percentage of species recorded in the unstructured data increased with commonness: 50% of very rare species were recorded, 90% of rare species were recorded, 100% of common and very common species were recorded.

< 10 species are missing from the unstructured data for this time period (i.e., excluding 2023 and 2024)
...
There are 50 species in the unstructured data that are not in the SABAP2 data. For 40 of these species there are < 15 records. For the 10 species for which there are > 15 records, these are the same species as discussed above. Reasons are discussed above.All comparisons below are based on the species in SABAP2.
...
If we look at the graph it appears that of the sabap2 squares the percentage of squares in which a species is observed in sabap2 is positively correlated to the percentage squares in which there is overlap between the sabap2 records and cube records. This correlation is weaker than for the period over 2022-2024.
...
For common and very common species (rare excluded) - If we look at the graph it appears that of the sabap2 squares the percentage of squares in which a species is observed in sabap2 is positively correlated to the percentage squares in which a species is recorded in the unstructured data. But these squares might not overlap, and the correlation is stronger when all species are included.
...
Graph shows for each species the degree to which the number of records over time (based on time periods) in SABAP2 correlates with number of records in unstructured dataset. The strength and direction of the correlation varies significantly between species, and for 38 species the correlation is NA. But, for > 20 species correlation = 1.
...
Either not strong or no correlation for all groups. Note most very rare species, many rare species and a few common species correlation was not possible. No pattern in terms of the correlations and commonness.
...
```{r}
# Give '1' to the squares each species is recorded in unstructured data
...
Low average occupancy accross species (25%). 25% of cells occupied in SABAP2 data are on average occupied in unstructured data
...
Positive and significant correlation between species richness calculated from the two datasets
