...
occ_download_sql(
  ...
  "SELECT \"year\", GBIF_EQDGCode(2, decimalLatitude, decimalLongitude,
                 ...
                 IF(ISNULL(\"order\"), NULL, SUM(COUNT(*)) OVER (PARTITION BY \"order\"))
                 AS orderCount, IF(ISNULL(family), NULL, SUM(COUNT(*)) OVER (PARTITION BY family)) AS familyCount,
                 IF(ISNULL(genus), NULL, SUM(COUNT(*)) OVER (PARTITION BY genus)) AS genusCount
                 FROM occurrence
                 WHERE class = 'Aves' AND
                 occurrenceStatus = 'PRESENT'AND
                 (coordinateUncertaintyInMeters <= 27000 OR coordinateUncertaintyInMeters IS NULL) AND NOT occurrence.basisofrecord IN ('FOSSIL_SPECIMEN', 'LIVING_SPECIMEN') AND NOT
...
# Note warnings are addressed in the code
...
SABAP2WC <- SABAP2WC %>%
  ...
  filter(fieldNotes != "3320_1900_012568_19000100") %>%
  filter(fieldNotes != "3325_1910_018033_20221029") %>%
  filter(fieldNotes != "3400_1935_018033_20230715")
```
...
WCSF$qdgccode <- apply(WCSFcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
fam <- SABAP2WCQDGCYear %>%
  group_by(qdgccode, family) %>%
  summarise(familycount = sum(n))

ord <- SABAP2WCQDGCYear %>%
  group_by(qdgccode, order) %>%
  summarise(ordercount = sum(n))

gen <- SABAP2WCQDGCYear %>%
  group_by(qdgccode, genus) %>%
  summarise(genuscount = sum(n))

SABAP2WCQDGCYear <- SABAP2WCQDGCYear %>%
  inner_join(fam) %>%
  inner_join(ord) %>%
  inner_join(gen)
```
...
# join datasets
SABAP2WCQDGCYearSamp <- SABAP2WCQDGCYear %>%
...
SABAP2WCPenYear <- SABAP2WCPenYear %>%
  inner_join(fam) %>%
  inner_join(ord) %>%
  inner_join(gen)
```
...
# join datasets
SABAP2WCPenYearSamp <- SABAP2WCPenYear %>%
...
```{r}
WCunstrucPenproc <- process_cube(birdcubeWCYearPentad,
...
```{r}
obs_WC_PenU <- birdcubeWCYearPentad %>%
  group_by(pentad) %>%
  summarise(Obs = sum(n))
```
...
```{r}
obs_WC_PenS <- SABAP2WCPenYearSamp %>%
...
```{r}
obs_WC_QDGCSeff <- SABAP2WCQDGCYearSamp %>%
...
map_obs_WC_QDGCU$data$qdgccode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
```{r}
obs_rich_WC_PenU <- WCunstrucPenproc$data %>%
  group_by(cellCode) %>%
...
```{r}
obs_rich_WC_PenS <- WCstrucPenproc$data %>%
  group_by(cellCode) %>%
  summarise(n_species = n_distinct(scientificName), .groups = "drop")
```
...
hill0_map_WC_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_WC_QDGCS$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_WC_QDGCSas$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
richEstsWCQDGCS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT - 0.945)",
...
```{r}
# Get qdgc codes
Codes <- map_obs_rich_WC_QDGCU$data %>%
  select(QDGCcode) %>%
  st_drop_geometry()
...
```{r}
inEXTestsWCPenS <- estimateD(WCPenincid,
  datatype = "incidence_freq",
  base = "coverage", level = NULL, conf = 0.95
) # If base="coverage" and level=NULL, then this function computes the diversity estimates for the minimum among the coverage values for samples extrapolated to double the size of the reference sample.
...
inEXTestsWCPenS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT - 0.93)",
    y = "Observed richness - unstructured data"
  )
```
...
hill1_map_WC_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill1_map_WC_QDGCS$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill1_map_WC_QDGCSas$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill1EstsWCQDGCS %>%
  ...
  labs(
    x = "Shannon Diversity - structured data (iNEXT - 0.945)",
...
hill1EstsWCQDGCS %>%
  ...
  labs(
    x = "Shannon Diversity - structured data (iNEXT - 0.94)",
    y = "Shannon Diversity - structured data (b3gbi - 0.95 (incidence))"
  )
```
...
hill2_map_WC_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill2_map_WC_QDGCS$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill2_map_WC_QDGCSas$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill2EstsWCQDGCS %>%
  ...
  labs(
    x = "Simpson Diversity - structured data (iNEXT - 0.945)",
...
hill0_map_WC_QDGCUAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_WC_QDGCSAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
richEstsWCQDGCS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT - 0.945 - incidence)",
...
richEstsWCQDGCS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT (incidence) - 0.945)",
...
hill1_map_WC_QDGCUAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill1_map_WC_QDGCSAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill1EstsWCQDGCS %>%
  ...
  labs(
    x = "Shannon Diversity - structured data (iNEXT (incidence) - 0.945)",
...
hill2_map_WC_QDGCUAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill2_map_WC_QDGCSAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill2EstsWCQDGCS %>%
  ...
  labs(
    x = "Simpson Diversity - structured data (iNEXT (incidence) - 0.945)",
