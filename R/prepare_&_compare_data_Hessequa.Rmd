...
occ_download_sql(
  user = "", pwd = "", email = "",
  "SELECT \"year\", GBIF_EQDGCode(2, decimalLatitude, decimalLongitude,
                 COALESCE(coordinateUncertaintyInMeters, 27000)) AS qdgcCode,
                 speciesKey, species,\"order\", family, genus, COUNT(*) AS n,
                 ...
                 FROM occurrence
                 WHERE class = 'Aves' AND
                 occurrenceStatus = 'PRESENT'AND
                 (coordinateUncertaintyInMeters <= 27000 OR coordinateUncertaintyInMeters IS NULL) AND NOT
                 occurrence.basisofrecord IN ('FOSSIL_SPECIMEN', 'LIVING_SPECIMEN') AND NOT
                 ARRAY_CONTAINS(issue, 'ZERO_COORDINATE') AND NOT
                 ARRAY_CONTAINS(issue, 'COORDINATE_OUT_OF_RANGE') AND NOT
                 ARRAY_CONTAINS(issue, 'COORDINATE_INVALID') AND NOT
                 ARRAY_CONTAINS(issue, 'COUNTRY_COORDINATE_MISMATCH') AND
                 level1Gid = 'ZAF.9_1'
                 AND \"year\" >= 2015
                 AND \"year\" <= 2023 A
                 ND speciesKey IS NOT NULL
...
```{r}
occ_download(
  user = "", pwd = "", email = "", pred("taxonKey", 212),
...
# Note the warnings are addressed in the code

birdsWC <- birdsWC %>%
  separate(decimalLatitude, c("degreesLat", "otherLat"), "\\.", remove = FALSE) %>% 
  mutate(otherLat = replace_na(as.numeric(otherLat), 0)) %>% 
...
birdsWC <- birdsWC %>%
  separate(decimalLongitude, c("degreesLong", "otherLong"), "\\.", remove = FALSE) %>% 
  mutate(otherLong = replace_na(as.numeric(otherLong), 0)) %>% 
...
birdsWC <- birdsWC %>% unite("pentad", c(pentadLat, pentadLong), sep = "_", remove = FALSE)
```
...
birdsHAA <- birdsWC %>%
  filter(birdsWC$pentad %in% Hpentads) %>%
  drop_na(speciesKey) %>%
  replace_na(list(coordinateUncertaintyInMeters = 8000))
```
...
birdcubeHAAYearPentad <- occurrence_cube_full %>%
  ...
  select(n:pentad, species:year) %>%
  left_join(MissDeats) %>%
  rename(mincoordinateuncertaintyinmeters = min_coord_uncertainty)

fam <- birdcubeHAAYearPentad %>%
  group_by(pentad, family) %>%
  summarise(familycount = sum(n))

ord <- birdcubeHAAYearPentad %>%
  group_by(pentad, order) %>%
  summarise(ordercount = sum(n))

gen <- birdcubeHAAYearPentad %>%
  group_by(pentad, genus) %>%
  summarise(genuscount = sum(n))

birdcubeHAAYearPentad <- birdcubeHAAYearPentad %>%
  inner_join(fam) %>%
  inner_join(ord) %>%
  inner_join(gen)
```
...
SABAP2HAA <- SABAP2HAA %>%
  distinct(species, fieldNotes, .keep_all = TRUE) %>%
  filter(fieldNotes != "3355_2045_013624_20201128")
```
...
Add a column with QDGC information. Need to use a function get_qdgc_code() to convert decimal degree co-ordinates to QDGC
...
# use get_qdgc_code function to get qdgc codes from co-ordinates
HAASF$qdgccode <- apply(HAASFcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
fam <- SABAP2HAAQDGCYear %>%
  group_by(qdgccode, family) %>%
  summarise(familycount = sum(n))

ord <- SABAP2HAAQDGCYear %>%
  group_by(qdgccode, order) %>%
  summarise(ordercount = sum(n))

gen <- SABAP2HAAQDGCYear %>%
  group_by(qdgccode, genus) %>%
  summarise(genuscount = sum(n))

SABAP2HAAQDGCYear <- SABAP2HAAQDGCYear %>%
  inner_join(fam) %>%
  inner_join(ord) %>%
  inner_join(gen)
```
...
# join datasets
SABAP2HAAQDGCYearSamp <- SABAP2HAAQDGCYear %>%
  left_join(QDGCSampLong, by = join_by(qdgccode, year)) %>%
  ungroup()
...
HAAQDGCSamp <- QDGCSamp %>%
  select(fullprotocol, qdgccode) %>%
  unique() %>%
...
# counts for genera, family, order
fam <- SABAP2HAAPenYear %>%
  group_by(pentad, family) %>%
  summarise(familycount = sum(n))

ord <- SABAP2HAAPenYear %>%
  group_by(pentad, order) %>%
  summarise(ordercount = sum(n))

gen <- SABAP2HAAPenYear %>%
  group_by(pentad, genus) %>%
  summarise(genuscount = sum(n))

SABAP2HAAPenYear <- SABAP2HAAPenYear %>%
  inner_join(fam) %>%
  inner_join(ord) %>%
  inner_join(gen)
```
...
# join datasets
SABAP2HAAPenYearSamp <- SABAP2HAAPenYear %>%
  left_join(PenSampLong, by = join_by(pentad, year)) %>%
  ungroup()
...
summary(HAAPenSC$DataInfo$SC) # summary of sample coverag
```
...
```{r}
SABAP2HAAQDGCYearSamp %>%
  select(year, qdgccode, totalYear) %>%
  unique() %>%
  group_by(year) %>%
  summarise(n = sum(totalYear)) %>%
...
```{r}
SABAP2HAAPenYearSamp %>%
  select(year, pentad, totalYear) %>%
  unique() %>%
  group_by(year) %>%
  summarise(n = sum(totalYear)) %>%
  ggplot(aes(x = year, y = n)) +
...
Overall we see a range overlap of 22%
...
```{r}
HAAunstrucQDGCproc <- process_cube(birdcubeHAAYearQDGC,
...
```{r}
HAAunstrucPenproc <- process_cube(birdcubeHAAYearPentad,
  grid_type = "custom", first_year = 2015, cols_year = "year", cols_cellCode = "pentad", cols_occurrences = "n",
  cols_minCoordinateUncertaintyInMeters = "mincoordinateuncertaintyinmeters", cols_familyCount = "familycount", cols_speciesKey = "speciesKey"
)
```
...
map_obs_HAA_QDGCS$data %>%
  ...
  labs(
    x = "Number of records - structured data",
...
```{r}
obs_HAA_PenU <- birdcubeHAAYearPentad %>%
  group_by(pentad) %>%
  summarise(Obs = sum(n))
```
...
```{r}
obs_HAA_PenS <- SABAP2HAAPenYearSamp %>%
  group_by(pentad, fullprotocol) %>%
  summarise(Obs = sum(n)) %>%
  mutate(Obseff = Obs / fullprotocol)
```
...
```{r}
obs_HAA_QDGCSeff <- SABAP2HAAQDGCYearSamp %>%
  group_by(qdgccode, fullprotocol) %>%
  summarise(Obs = sum(n)) %>%
  mutate(Obseff = Obs / fullprotocol)
```
...
map_obs_HAA_QDGCU$data$qdgccode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
obs_HAA_QDGCSeff %>%
  ...
  labs(
    x = "Number of records/number of surveys - structured data",
    y = "Number of records - unstructured data"
  )
```
...
```{r}
obs_rich_HAA_PenU <- HAAunstrucPenproc$data %>%
  group_by(cellCode) %>%
...
```{r}
obs_rich_HAA_PenS <- HAAstrucPenproc$data %>%
  group_by(cellCode) %>%
  summarise(n_species = n_distinct(scientificName), .groups = "drop")
```
...
```{r}
inEXTestsHAAQDGCS <- estimateD(HAAQDGCincid,
...
map_obs_rich_HAA_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_HAA_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_HAA_QDGCS$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_HAA_QDGCSas$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
richEstsHAAQDGCS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT - 0.995)",
...
```{r}
# Get qdgc codes
Codes <- map_obs_rich_HAA_QDGCU$data %>%
  select(QDGCcode) %>%
  st_drop_geometry()
...
```{r}
hill0_map_HAA_QDGCU15$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
```{r}
inEXTestsHAAPenS <- estimateD(HAAPenincid,
  datatype = "incidence_freq",
  base = "coverage", level = NULL, conf = 0.95
) # If base="coverage" and level=NULL, then this function computes the diversity estimates for the minimum among the coverage values for samples extrapolated to double the size of the reference sample.
...
inEXTestsHAAPenS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT - 0.98)",
    y = "Observed richness - unstructured data"
  )
```
...
hill1_map_HAA_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill1_map_HAA_QDGCS$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill1_map_HAA_QDGCSas$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill1EstsHAAQDGCS %>%
  ...
  labs(
    x = "Shannon Diversity - structured data (iNEXT - 0.995)",
...
```{r}
# Get qdgc codes
Codes <- hill1_map_HAA_QDGCU$data %>%
  select(QDGCcode) %>%
  st_drop_geometry()
...
```{r}
hill1_map_HAA_QDGCU45$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill2_map_HAA_QDGCU$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill2_map_HAA_QDGCS$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill2_map_HAA_QDGCSas$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill2EstsHAAQDGCS %>%
  ...
  labs(
    x = "Simpson Diversity - structured data (iNEXT - 0.995)",
...
```{r}
# Get qdgc codes
Codes <- hill2_map_HAA_QDGCU$data %>%
  select(QDGCcode) %>%
  st_drop_geometry()
...
```{r}
hill2_map_HAA_QDGCU45$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill0_map_HAA_QDGCUAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill0_map_HAA_QDGCSAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
richEstsHAAQDGCS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT - 0.995 - incidence)",
...
richEstsHAAQDGCS %>%
  ...
  labs(
    x = "Estimated richness - structured data (iNEXT (incidence) - 0.995)",
...
```{r}
hill0_map_HAA_QDGCU55Ab$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill1_map_HAA_QDGCUAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill1_map_HAA_QDGCSAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill1EstsHAAQDGCS %>%
  ...
  labs(
    x = "Shannon Diversity - structured data (iNEXT (incidence) - 0.995)",
...
```{r}
hill1_map_HAA_QDGCU15Ab$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill2_map_HAA_QDGCUAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))

hill2_map_HAA_QDGCSAb$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
...
hill2EstsHAAQDGCS %>%
  ...
  labs(
    x = "Simpson Diversity - structured data (iNEXT (incidence) - 0.995)",
...
```{r}
hill2_map_HAA_QDGCU05Ab$data$QDGCcode <- apply(Hillcoords[, c("Y", "X")], 1, function(y) get_qdgc_code(y["Y"], y["X"]))
```
